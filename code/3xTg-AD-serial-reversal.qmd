...
On the last third there was a significant effect of genotype (*F*~(`r format(last_third_aov$DFn[1], digits = 2)`,`r format(last_third_aov$DFd[1], digits = 2)`)~ = `r format(last_third_aov$F[1], digits = 2)`, *p* `r format.p(last_third_aov$p[1], digits = 2)`, , $\eta^2_G$ = `r format(last_third_aov$ges[1], digits = 2)`), 
but no effect of odour pair (*F*~(`r format(last_third_aov$DFn[2], digits = 2)`,`r format(last_third_aov$DFd[2], digits = 2)`)~ = `r format(last_third_aov$F[2], digits = 2)`, *p* `r format.p(last_third_aov$p[2], digits = 2)`, $\eta^2_G$ = `r format(last_third_aov$ges[2], digits = 2)`), 
nor an interaction (*F*~(`r format(last_third_aov$DFn[3], digits = 2)`,`r format(last_third_aov$DFd[3], digits = 2)`)~ = `r format(last_third_aov$F[3], digits = 2)`, *p* `r format.p(last_third_aov$p[3], digits = 2)`, $\eta^2_G$ = `r format(last_third_aov$ges[3], digits = 2)`).
...
total_errors_t <- total_errors_dat %>% 
    ungroup() %>% 
    t_test(total_errors ~ geno)
...
A t-test was used to assess the total errors made during the discrimination and reversal trials.
There was a significant effect of genotype (*t*~(`r format(total_errors_t$df, digits = 2)`)~ = `r format(total_errors_t$statistic, digits = 2)`, *p* `r format.p(total_errors_t$p, digits = 2)`; @fig-total-errors).
...
A t-test was used to assess the number of errors made during the retest. 
There was no significant effect of genotype (*t*~(`r format(retest_t$df, digits = 2)`)~ = `r format(retest_t$statistic, digits = 2)`, *p* `r format.p(retest_t$p, digits = 2)`; @fig-retest-errors)
