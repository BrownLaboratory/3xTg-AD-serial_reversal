...
auto_data <- auto_files %>% 
    ...
    mutate(animal = case_when(
            animal == "1297" ~ "1927", # fix typo in animal number
            TRUE ~ animal),
...
Male and female 3xTg-AD (B6;129-Tg(APPSwe,tauP301L)1Lfa Psen1^tm1Mpm^/Mmjax; JAX stock #004807) and B6129F2/J mice (JAX stock #101045) were bred at Dalhousie University from parents purchased from the Jackson Laboratory in Bar Harbour, Maine.
...
The B6129F2/J mice are the offspring of an F1 x F1 mating, itself the product of a cross of C57BL/6J females and 129S1/SvImJ males, and are the suggested controls for 3xTg-AD mice.
...
There were `r dat %>% pull(animal) %>% unique() %>% length()` mice included in this study and mice were randomly selected from the colony for testing, resulting in a wide range of ages tested (@tbl-subjects).
...
The 3xTg-AD mice (`r format(age_table$age_mean[1], digits = 2)` ± `r format(age_table$age_sd[1], digits = 2)` days) were significantly younger than the B6129F2/J mice (`r format(age_table$age_mean[2], digits = 2)` ± `r format(age_table$age_sd[2], digits = 2)` days; *t*~`r format(t_test_age$df, digits = 2)`~ = `r format(t_test_age$statistic, digits = 2)`, *p* `r format.p(t_test_age$p)`).
This difference in ages is likely the result of the random selection of mice from the colony, combined with the fact that 3xTg-AD mice have a shorter lifespan than B6129F2/J mice [@raeProblemGenotypeSex2015].
Due to the difference in ages, ANCOVAs with age as a covariate were used to control for age differences, and age was correlated with errors made to examine age related changes in error rates.
...
#| fig-cap: "Number of errors made during odour discrimination training by 3xTg-AD and B6129F2/J mice. The boxes represent the inter-quartile range (IQR), the bars in the middle of each box show the medians, the boarders of each box shows the 25th and 75th percentiles, and the whiskers extending to the furthest points within 1.5 interquartile ranges"
...
training_dat %>% 
    ...
    labs(y = "Errors",
         ...
         caption = "Number of errors made during odour discrimination training by 3xTg-AD and B6129F2/J mice.\nThe boxes represent the inter-quartile range (IQR), the bars in the middle of each box show\nthe medians, the boarders of each box shows the 25th and 75th percentiles, and the whiskers\nextending to the furthest points within 1.5 interquartile ranges") +
...
dat_text <- tibble(geno = c("B6129F2/J", "3xTg-AD"),
                   geno_name = c("B6129F2/J", "3xTg-AD"),
                   condition = c("disc", "rev"))
...
```{r}
#| label: fig-errorless
#| fig-cap: "Number of 3xTg-AD and B6129F2/J mice showing errorless learning on each odour pair during A) discrimination learning, and B) reversal learning."
#| fig-height: 6
...
There were `r disc_1_error %>% nrow()` times where mice made a single error on the initial discrimination of the odour pair, and `r disc_0_error %>% nrow()` times where a mouse made zero errors (@fig-errorless A), with `r disc_errorless_table %>% length()` different mice learning at least one discrimination with just one or zero errors.
...
fig_total_disc_errors <- disc_dat %>% 
...
However, these effects were driven by one female B6129F2/J mouse who made 231 errors on the reversal of odour pair 17.
...
There were `r rev_1_error %>% nrow()` instances of mice making a single error on the reversal of the odour pair, and `r rev_0_error %>% nrow()` instances of a mouse making zero errors (@fig-errorless B), with `r rev_errorless_table %>% length()` mice learning at least one reversal with just one or zero errors.
...
The effect of genotype was significant ($\chi^2$~`r format(rev_errorless_chi_geno$parameter, digits = 2)`~ = `r format(rev_errorless_chi_geno$statistic, digits = 2)`, *p* `r format.p(rev_errorless_chi_geno$p.value)`), with more 3xTg-AD mice having errorless learning than the B6129F2/J mice.
...
plot_grid(fig_total_disc_errors, fig_total_rev_errors,
          nrow = 2)


ggsave(here("figures", "total_disc_rev_errors_plot.pdf"),
       width = 6,
       height = 6)
```
...
Pearson's correlations between the age of the mice and the difference scores were not significant for either the B6129F2/J (*r* = `r format(dif_corr_test$cor[2], digits = 2)`, *p* `r format.p(dif_corr_test$p[2])`), nor the 3xTg-AD (*r* = `r format(dif_corr_test$cor[1], digits = 2)`, *p* `r format.p(dif_corr_test$p[1])`) mice (@fig-age-diff-corr).
...
retest_age_range <- retest_dat %>% 
    mutate(retest_age = retest_date - dob) %>% 
    group_by(geno) %>% 
    summarise(min = min(retest_age),
              max = max(retest_age))
```

Because 3xTg-AD mice have a shorter lifespan than B6129F2/J mice [@raeProblemGenotypeSex2015], fewer 3xTg-AD mice (`r retest_table$n[1]` / `r subj_n_table$n[1]`) survived to be given the retest than the B6129F2/J mice (`r retest_table$n[2]` / `r subj_n_table$n[2]`).
This also resulted in different age ranges on the retest, with the 3xTg-AD mice have ages from `r retest_age_range$min[1] %>% as.numeric()` to `r retest_age_range$max[1] %>% as.numeric()` days while the B6129F2/J mice had ages ranging from `r retest_age_range$min[2] %>% as.numeric()` to `r retest_age_range$max[2] %>% as.numeric()` days.

There was no significant difference in the number of days between the last reversal task and the retest for the B6129F2/J (`r format(retest_summary$days_mean[2], digits = 2)` ± `r format(retest_summary$days_sd[2], digits = 2)`) and 3xTg-AD mice (`r format(retest_summary$days_mean[1], digits = 2)` ± `r format(retest_summary$days_sd[1], digits = 2)`; *t*~`r format(retest_date_t_test$df, digits = 2)`~ = `r format(retest_date_t_test$statistic, digits = 2)`, *p* `r format.p(retest_date_t_test$p)`).
...
Pearson correlations showed no significant relationship between the age of the mice and the number of errors made on the retest for the B6129F2/J mice (*r* = `r format(retest_corr_test$cor[2], digits = 2)`, *p* `r format.p(retest_corr_test$p[2])`), but for the 3xTg-AD mice there was a significant decrease with age (*r* = `r format(retest_corr_test$cor[1], digits = 2)`, *p* `r format.p(retest_corr_test$p[1])`; @fig-retest-age-corrs).
...
Although all mice showed an increase in the number of errors, Pearson correlations showed a significant, positive correlation for the B6129F2/J mice on the number of errors made on the retest to the days since the final reversal (*r* = `r format(retest_days_corr_test$cor[2], digits = 2)`, *p* `r format.p(retest_days_corr_test$p[2])`), but not for the 3xTg-AD mice (*r* = `r format(retest_days_corr_test$cor[1], digits = 2)`, *p* `r format.p(retest_days_corr_test$p[1])`; @fig-retest-errors-corrs B).
...
```{r}
#| label: fig-retest-age-corrs
#| fig-cap: "Correlation between age of the 3xTg-AD and B6129F2/J mice and the number of errors made on the retest."
#| fig-height: 6
...
```{r}
#| label: fig-retest-errors-corrs
#| fig-cap: "(A) Total errors made during reversal learning of odour pair 18 and the retest by 3xTg-AD and B6129F2/J mice. The boxes represent the inter-quartile range (IQR), the bars in the middle of each box show the medians, the borders of each box show the 25th and 75th percentiles, and the whiskers extend to the furthest points within 1.5 interquartile ranges. (B) Correlation between days since the reversal learning of pair 18 and the number of errors made on the retest for 3xTg-AD and B6129F2/J mice."
#| fig-height: 6
brks <- seq(0, 120, 10)
lbs <- ifelse(brks %% 20 == 0, brks, "")
...
plot_grid(fig_retest_errors,
          retest_delay_cor_plot,
          nrow = 2)

# retest_delay_cor_plot

ggsave(here("figures", "retest_errors_corrs.pdf"),
       width = 6,
       height = 8)
```
...
The variances did not differ between genotypes on the total errors made during discrimination trials (*W*~(`r format(levene_disc$df1, digits = 2)`,`r format(levene_disc$df2, digits = 2)`)~ = `r format(levene_disc$statistic, digits = 2)`, *p* `r format.p(levene_disc$p)`), reversal trials (*W*~(`r format(levene_rev$df1, digits = 2)`,`r format(levene_rev$df2, digits = 2)`)~ = `r format(levene_rev$statistic, digits = 2)`, *p* `r format.p(levene_rev$p)`), nor on the total number of errors made (*W*~(`r format(levene_total_errors$df1, digits = 2)`,`r format(levene_total_errors$df2, digits = 2)`)~ = `r format(levene_total_errors$statistic, digits = 2)`, *p* `r format.p(levene_total_errors$p)`).
The 3xTg-AD mice (`r total_errors_table$diff[total_errors_table$geno == "3xTg-AD"]`) and B6129F2/J mice (`r total_errors_table$diff[total_errors_table$geno == "B6129F2/J"]`) had similar ranges between the best (3xTg-AD = `r total_errors_table$min[total_errors_table$geno == "3xTg-AD"]`; B6129F2/J = `r total_errors_table$min[total_errors_table$geno == "B6129F2/J"]`) and worst (3xTg-AD = `r total_errors_table$max[total_errors_table$geno == "3xTg-AD"]`; B6129F2/J = `r total_errors_table$max[total_errors_table$geno == "B6129F2/J"]`) performing mice in terms of total errors made.
