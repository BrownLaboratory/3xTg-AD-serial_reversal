...
Post hoc Tukey HSD tests found that there were significant genotype differences on odour pairs 2 (adjusted *p* `r format.p(rev_first_third_ph$p.adj[2])`) and 6 (adjusted *p* `r format.p(rev_first_third_ph$p.adj[6])`).
...
```{r}
rev_errorless_chi_op <- tabyl(rev_errorless_dat, op, errorless) %>%
    chisq.test()
