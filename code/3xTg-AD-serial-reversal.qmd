...
training_summmary <- training_dat %>% 
    group_by(geno) %>% 
...
overall_dat <- dat %>% 
    filter(retest == FALSE,
           op > 0) %>% 
    pivot_wider(id_cols = c(animal, op, geno),
                names_from = rev,
                values_from = errors) %>% 
    select(animal,
           op,
...
The sequence of odour pairs was divided into thirds, and ANOVAs examining the  the errors on the discrimination trials were run on each third.
Greenhouse-Geisser corrections were applied when Mauchly's test for sphericity detected that within-subjects factors violated sphericity.
...
In the second third of trials there were no significant effects (*p*'s >=`r format(min(disc_second_third_aov$p), digits = 2)`).
...
```{r}
disc_errorless_chi_op <- tabyl(disc_errorless_dat, op, errorless) %>%
    chisq.test()
...
<!-- During the second half of the experiment, odour pairs 10 through 18, the B6129 made an average of `r format(disc_second_half$errors_mean[2], digits = 2)` ± `r format(disc_second_half$errors_sd[2], digits = 2)` errors,  -->
<!-- while the 3xTg-AD mice made `r format(disc_second_half$errors_mean[1], digits = 2)` ± `r format(disc_second_half$errors_sd[1], digits = 2)` errors. -->
<!-- This difference was not significant (*t*~`r format(disc_second_half_t_test$df, digits = 2)`~ = `r format(disc_second_half_t_test$statistic, digits = 2)`, *p* `r format.p(disc_second_half_t_test$p)`). -->
...
During the second half of the experiment, odour pairs 10 through 18, the B6129 made an average of `r format(rev_second_half$errors_mean[2], digits = 2)` ± `r format(rev_second_half$errors_sd[2], digits = 2)` errors, 
while the 3xTg-AD mice made `r format(rev_second_half$errors_mean[1], digits = 2)` ± `r format(rev_second_half$errors_sd[1], digits = 2)` errors, a significant difference (*t*~`r format(rev_second_half_t_test$df, digits = 2)`~ = `r format(rev_second_half_t_test$statistic, digits = 2)`, *p* `r format.p(rev_second_half_t_test$p)`).
...
There was a significant, positive correlations (*r* = `r format(retest_days_corr_test$cor, digits = 2)`, *p* `r format.p(retest_days_corr_test$p)`).